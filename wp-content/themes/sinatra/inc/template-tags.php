...
if ( ! function_exists( 'sinatra_entry_meta_author' ) ) :
	...
	function sinatra_entry_meta_author( $args = array() ) {
		...
		$defaults = array(
			'show_avatar' => sinatra_option( 'entry_meta_icons' ),
...
if ( ! function_exists( 'sinatra_entry_meta_date' ) ) :
	...
	function sinatra_entry_meta_date( $args = array() ) {
		...
		// Icon.
		$icon = sinatra_option( 'entry_meta_icons' ) ? '<i class="si-icon si-clock" aria-hidden="true"></i>' : '';
		$icon = apply_filters( 'sinatra_date_meta_icon', $icon );
...
if ( ! function_exists( 'sinatra_entry_meta_comments' ) ) :
	...
	function sinatra_entry_meta_comments() {
		...
		$icon = sinatra_option( 'entry_meta_icons' ) ? '<i class="si-icon si-message-square" aria-hidden="true"></i>' : '';
		$icon = apply_filters( 'sinatra_comments_meta_icon', $icon );
...
if ( ! function_exists( 'sinatra_entry_meta_category' ) ) :
	...
	function sinatra_entry_meta_category( $sep = ', ', $show_icon = true, $return = false ) {
		...
		// Icon.
		if ( $show_icon && sinatra_option( 'entry_meta_icons' ) ) {
			$show_icon = apply_filters( 'sinatra_category_meta_icon', '<i class="si-icon si-bookmark" aria-hidden="true"></i>' );
		} else {
			$show_icon = '';
		}
...
if ( ! function_exists( 'sinatra_entry_meta_tag' ) ) :
	...
	function sinatra_entry_meta_tag( $before = '<span class="cat-links"><span>', $sep = ', ', $after = '</span></span>', $id = 0, $show_icon = true ) {
		...
		if ( $show_icon && sinatra_option( 'entry_meta_icons' ) ) {
			$show_icon = apply_filters( 'sinatra_tags_meta_icon', '<i class="si-icon si-tag" aria-hidden="true"></i>' );
		} else {
			$show_icon = '';
		}

		// Add icon.
		$before = $before . wp_kses_post( $show_icon );
