...
if ( ! class_exists( 'Sinatra_Customizer' ) ) :
	...
	class Sinatra_Customizer {
		...
		public function register_options_new( $customizer ) {
			...
			$options = $this->get_customizer_options( $customizer );
		 ...
		 * @since 1.0.0
		 * 
		 * @param WP_Customize_Manager $customizer Reference to WP_Customize_Manager.
		 * 
		 * @return Array Customizer options for registering Sections/Panels/Controls.
		 */
		private function get_customizer_options( $customizer ) {
			if ( ! is_null( self::$options ) ) {
				return self::$options;
			}

			return apply_filters( 'sinatra_customizer_options', array(), $customizer );
		}
