...
;( function( $ ) {
	...
	api( 'sinatra_single_narrow_container_width', function( value ) {
		value.bind( function( newval ) {
			...
			style_css += '.single-post.narrow-content .entry-content > :not([class*="align"]):not([class*="gallery"]):not(.wp-block-image):not(.quote-inner):not(.quote-post-bg), ' +
					...
					'.single-post.narrow-content.si-single-layout-1 .entry-header, ' +
					'.single-post.narrow-content.si-single-layout-1 .entry-meta, ' +
					'.single-post.narrow-content.si-single-layout-1 .post-category, ' +
					'.single-post.narrow-content.sinatra-no-sidebar .si-page-header-wrapper, ' +
					'.single-post.si-single-layout-2.sinatra-no-sidebar.narrow-content .si-breadcrumbs {' +
	...
	api( 'sinatra_main_nav_font_size', function( value ) {
		value.bind( function( newval ) {
			...
			style_css += sinatra_range_field_css( '#sinatra-header-inner', 'font-size', newval, false, newval.unit );
	...
	api( 'sinatra_hero_hover_slider_container', function( value ) {
		value.bind( function( newval ) {
			...
			if ( 'full-width' === newval ) {
				$hero_container.removeClass( 'si-container' ).addClass( 'si-container__wide' );
			} else {
				$hero_container.removeClass( 'si-container__wide' ).addClass( 'si-container' );
			}
	...
	api( 'sinatra_pre_footer_cta_font_size', function( value ) {
		value.bind( function( newval ) {
			...
			var style_css = sinatra_range_field_css( '#si-pre-footer .h2', 'font-size', newval, true, newval.unit );
	 ...
	 * Page Header layout.
	 */
	api( 'sinatra_page_header_layout', function( value ) {
	 	value.bind( function( newval ) {
	 		...
	 		if ( $page_header.length ) {
	 			if ( 'center' === newval ) {
	 				$page_header.addClass( 'centered-layout' );
	 			} else {
	 				$page_header.removeClass( 'centered-layout' );
	 			}
	 		}
		});
	});
	 ...
	 */
	api( 'sinatra_page_header_height', function( value ) {
	 	value.bind( function( newval ) {
	
	 		$style_tag = sinatra_get_style_tag( 'sinatra_page_header_height' );

	 		var style_css  = sinatra_range_field_css( '.si-page-header-wrapper', 'min-height', newval, true, 'px' );
	...
	api( 'sinatra_page_header_text_color', function( value ) {
		value.bind( function( newval ) {
			...
			style_css += '.page-header .si-page-header-description, .si-single-layout-2 .page-header .entry-meta' +
						 '{ color: ' + sinatra_hex2rgba( newval['text-color'], 0.75 ) + '}';

			style_css += '.si-single-layout-2 .page-header .entry-meta .entry-meta-elements>span:before {' +
						 'background-color: ' + sinatra_hex2rgba( newval['text-color'], 0.5 ) + ';}';
			// Link color.
			...
			// Link hover color.
			style_css += '.page-header .si-breadcrumbs a:hover, .si-single-layout-2 .page-header .entry-meta a:hover' +
						 '{ color: ' + newval['link-hover-color'] + '; }';

			// Category links.
			style_css += '.si-single-layout-2.single .page-header .post-category a ' +
						 '{ color: ' + newval['link-color'] + '; border-color: ' + newval['link-color'] + '; }';

			style_css += '.si-single-layout-2.single .page-header .post-category a:hover ' +
						 '{ background-color: ' + newval['link-color'] + '; }';
