...
function sinatra_get_sidebar_position( $post = null ) {
	...
	return apply_filters( 'sinatra_sidebar_position', $sidebar_position );
}
...
function sinatra_get_single_post_elements( $post_id = null ) {
	...
	if ( 'quote' !== $post_format ) {
		...
		$layout = sinatra_option( 'single_post_layout' );
		$layout = apply_filters( 'sinatra_single_post_layout', $layout );

		if ( 'layout-1' === $layout ) {

			if ( sinatra_option( 'single_post_categories' ) ) {
				$elements[] = 'category';
			}
		...
		if ( sinatra_show_post_thumbnail( $post_id ) && sinatra_option( 'single_post_thumb' ) ) {
			$elements[] = 'thumbnail';
		}
	...
	if ( sinatra_option( 'single_about_author' ) ) {
		$elements[] = 'about-author';
	}

	if ( sinatra_option( 'single_post_next_prev' ) ) {
		$elements[] = 'prev-next-post';
	}

	$elements = apply_filters( 'sinatra_single_post_elements', $elements );
...
function sinatra_page_header_has_breadcrumbs( $post_id = 0 ) {
	...
	if ( $display && get_post_meta( $post_id, 'sinatra_disable_breadcrumbs', true ) ) {
		$display = false;
	}

	return apply_filters( 'sinatra_page_header_has_breadcrumbs', $display );
}
...
function sinatra_page_header_has_title( $post_id = 0 ) {
	...
	if ( is_singular( 'post' ) && ! in_array( sinatra_option( 'single_post_layout' ), array( 'layout-2' ), true ) ) {
		$display = false;
	}
...
function sinatra_page_header_classes( $classes = array() ) {
	...
	// Layout.
	if ( 'center' === sinatra_option( 'page_header_layout' ) ) {
...
function sinatra_body_classes( $classes ) {
	...
	if ( is_home() || is_archive() || is_search() ) {
		...
		if ( '' !== $sinatra_article_feed_layout ) {
			$classes[] = 'sinatra-' . sinatra_get_article_feed_layout();
		}
	}

	// Single post.
	if ( is_singular( 'post' ) ) {
		$classes[] = 'si-single-' . sinatra_option( 'single_post_layout' );
